...
public:
	...
	{
		//std::u16string str
		std::u16string str{(const char16_t*)il2cpp_string_chars((intptr_t)this)};
...
public:
	...
	String* ToString() {
		if (this) {
			return Domain::GetCorlib()
				->GetClassFromName("System", "Object")
				->GetMethodFromName("ToString", 0)
				->Invoke<String*>(this, { }, false);
		}
		return NULL;
	}

	std::string ToString_c() {
		String* str = ToString();
		if (str) {
			return str->Read();
		}
...
struct Exception {
	const char* ReadMessage()
	{
		...
		char ourMessageBytes[bufferSize];
		il2cpp_format_exception((intptr_t)this, ourMessageBytes, bufferSize);
		std::string str(ourMessageBytes);
		il2cpp_format_stack_trace((intptr_t)this, ourMessageBytes, bufferSize);
		std::string err = str + std::string(ourMessageBytes);
		return err.c_str();
	}
};
